...
| `GUARDRAILS_CONFIG` | Optional inline policy and local NeMo checks endpoint; when present, input and output checks are mandatory. See [guardrails](GUARDRAILS.md). |
...
- `POST /v1/chat/completions` — without guardrail configuration, the body is forwarded unchanged over the connection the gateway check pinned, the caller's bearer replaced by the RedPill key, status/content-type/stream relayed. With guardrails enabled, input is checked before generation and the entire completion is checked before release; streaming is buffered. `404 model_not_found` outside the allowlist, checked before any upstream contact; `502 upstream_unverified` when the gateway check fails; `502 upstream` when the pinned connection itself fails. See [guardrail errors and limits](GUARDRAILS.md#api-behavior-and-limits).
- `GET /healthz` (process only) needs no bearer. `GET /ready` requires the same caller bearer as `/v1`, before acquiring guardrail capacity or contacting dependencies. Configure readiness monitors to send `Authorization: Bearer <caller-bearer>`. Readiness requires the default model's gateway verification and, when enabled, successful input and output guardrail probes.
...
Guardrail tests exercise the checks API contract with a local fake: both phases, incomplete or malformed verdicts, deadlines, redirects, capacity limits, unsupported content, credentials, provider verification, and withholding JSON/SSE/tool arguments until approval. They do not measure classifier accuracy or exercise live NVIDIA services.
