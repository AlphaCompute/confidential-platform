...
- `/ready` requires the existing caller bearer before acquiring guardrail capacity, then performs both checks on synthetic probe text and checks provider attestation. Configure monitors with `Authorization: Bearer <caller-bearer>` and an appropriate polling interval. Missing or invalid credentials receive `401` without consuming checker capacity. `/healthz` stays a cheap, unauthenticated liveness check.
