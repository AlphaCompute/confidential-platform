...
Two CVMs on node `prod5` (US-WEST-1, dstack host v0.6.0), each deployed on 2026-09-14 and deleted right after its capture:
...
Between the two, `mrtd` and `rtmr1` are identical; `rtmr0` differs with the CVM shape and `rtmr2` with the image, so a reference value is one row per image and shape, as `docs/kms-spec.md` §4.3 says. The same procedure produced both:
...
2. Deploy: `phala deploy -n attest-capture -c capture/docker-compose.yml -t tdx.small --wait` for the dev capture, `phala deploy -n attest-capture-prod -c capture/docker-compose.yml -t tdx.large --image dstack-0.5.9 --no-dev-os --wait` for the production one (the `phala` CLI, `npx phala@latest`, with `PHALA_CLOUD_API_KEY` set).
